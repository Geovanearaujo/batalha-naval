...
(define (refresh-canvas canvas color)
  ...
  (define (go-ships L)
    (cond ((not (null? L))
           (begin
             (send (send canvas get-dc) set-pen "black" 1 'solid)
             (send (send canvas get-dc) set-brush "black" 'solid)
...
(define frame (new frame% [label "Battleship"] [width 2000] [height 1000]))
...
(define field-canvas (cons (new-field-canvas (car playgrounds) "DodgerBlue")
